...
description: "Turn customer calls, product reviews, and launch planning into capability briefs, customer-facing copy, launch announcements, and messaging pillars."
...
- **Confirm verbatim quotes are accurate.** Verify against the source. Quotes that misrepresent the speaker damage trust; quotes that capture exactly what was said create credibility. If a quote came through Chat from a meeting a teammate recorded, you only see the excerpt; ask whoever recorded it to confirm the context before it ships.
...
Many marketing artifacts draw from multiple source conversations. A launch announcement pulls from the launch planning meeting plus the strategic narrative discussion plus customer reviews. A messaging pillar document synthesizes across customer calls plus exec strategy plus competitive intelligence. A battle card pulls from sales calls plus win/loss interviews plus product reviews. [Chat](/earmark-basics/chat) handles this: it answers across meetings and cites each one.

**Group source meetings in a project.** When multiple meetings feed a single artifact, give the artifact a [project](/earmark-basics/projects) (*"Q3 launch — multi-region"*) and add every source meeting to it. Put your brand voice block in the project's **Instructions** (under **Project settings**) so every answer follows it. Then paste the template in as a question in the project chat. The output draws from all of the meetings, with source chips pointing back to them.

The writer is rarely the person who recorded the customer calls. You can only add meetings you recorded, so share the project with the AEs, researchers, and CSMs who ran them and ask each to add their own. People in the project see each meeting's title, who recorded it, and when. They can't open each other's meetings, but Chat can use them to answer questions in the project and shows short excerpts as sources.

**The voice-of-customer extraction.** A specifically marketing use of multi-meeting synthesis. Across many customer interviews, extract the language patterns customers use to describe a problem space. A standing voice-of-customer project, where researchers and AEs add their interviews as they happen, keeps the source set ready:

```md
Across the customer interviews in this project, identify the most common
...
Name the period when the corpus should reach further back. Check the **Coverage** line for how many interviews it reviewed.
...
Across the customer conversations in this project, identify:
...
- [Projects](/earmark-basics/projects) — grouping source meetings by launch or artifact, including teammates' calls
- [Chat](/earmark-basics/chat) — synthesis across source meetings, with sources
