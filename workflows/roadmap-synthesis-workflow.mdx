...
description: "Turn customer research, strategy discussions, and capacity reviews into a prioritized roadmap with clear reasoning, and keep it current as things change."
...
Roadmap synthesis is the most cross-meeting-dependent workflow in this guide series. By definition, it draws across customer research from many calls, strategy discussions from leadership meetings, capacity reviews from engineering syncs, and competitive intelligence from market discussions.

The default path is a [project](/earmark-basics/projects) plus [Chat](/earmark-basics/chat). Collect the source meetings in one roadmap project, then run each synthesis from the project page. Project chat answers across every meeting in the project, including the ones teammates shared, cites the meetings it drew on with source chips, and follows the project's **Instructions**. Each focused source-layer synthesis (customer signal across recent calls, strategy across recent leadership meetings, capacity across engineering reviews) is its own question in project chat. The integrated roadmap is then built from the four layered outputs.

Two habits keep the synthesis honest. Name the period when you want a longer window (a half-year of strategy discussions, a full year for the annual rebuild). And read the **Coverage** line at the end of each answer: it tells you the date range searched and how many meetings were reviewed, which is the first thing to check before you trust a frequency count.

This is still substantial work — 2–4 hours for an initial build, 30–60 minutes for monthly refreshes. Chat removes the context-assembly step; the time that's left is judgment: verifying signal, pressure-testing commitments, deciding what not to pursue. For most product organizations, that investment is worth it. The cost of doing this work badly across a year of mis-prioritized execution is much larger than the cost of doing it well across a few hours of synthesis per refresh.

If you want to run your own analysis instead (for example, a year of customer research across hundreds of calls pushed through your own scripts), [exporting local transcripts](/earmark-basics/local-transcripts) and running an external agent is still an option.
...
A roadmap built from a single source has predictable blind spots. The synthesis is what gives the roadmap legitimacy across audiences. Identify which of these sources are captured in Earmark — those become queryable substrate once they're in the roadmap project. Sources that aren't captured need explicit notes in the roadmap about where the signal came from.
...
- New source meetings since the last update (in project chat, the
  project's meetings since {previous update date}; otherwise paste
  artifacts below)
...
<Steps>
<Step title="Collect the source meetings in a roadmap project">
Customer research calls from the last one to three months; strategy / leadership discussions from the last one to two quarters; engineering capacity reviews from the last quarter; sales win/loss interviews; CS / renewal calls; competitive intelligence sessions.

Create a [project](/earmark-basics/projects) (e.g., "Roadmap Q2 2026") and add them. On the **Meetings** page, tick the meetings you want (Shift-click for a range, up to 50 at a time) and choose **Add to project**. You can only add meetings you recorded, so **Share** the project with the sales, CS, and engineering leads as collaborators and ask them to add theirs. New calls can join the project as they happen, including before they start.

In **Project settings**, put your time-horizon framework and prioritization framework in **Instructions**, so every answer in the project uses them.
</Step>
<Step title="Run the four-layer focused synthesis">
The four-layer pattern (covered below) — run each layer's focused synthesis first, each as its own question in project chat. Each produces a section that feeds Template A.
</Step>
<Step title="Run Template A against the integrated input">
In project chat, paste Template A along with the four layered outputs. Chat draws on the layers and on the source meetings behind them. First draft will be long and rough — that's expected.

If you'd rather run Template A as a meeting task, paste the layered outputs and key source artifacts into a **Customize context** document and run it there.
</Step>
...
- **Verify customer signal claims.** For each *"this came up in N customer calls"* claim, spot-check at least a few: hover the source chips for the excerpts, and compare N against the **Coverage** line. The model occasionally over- or under-counts.
...
Reviewers who are in the roadmap project can check claims against the same meetings you used: they ask project chat and get cited excerpts, even from calls they didn't record.
</Step>
...
The most distinctive technique in this workflow. Each layer is a focused synthesis run independently, then integrated. In practice, each layer is a separate question in project chat, so each answer stays focused and cites its own sources.
...
**Annual deep rebuild.** Once a year, rebuild from scratch rather than iterating. Make sure the full year's meetings are in the project and name the full year when you ask; run Template A fresh; compare against the current incremental roadmap. The annual rebuild prevents the slow drift that monthly iteration produces. Sometimes the current roadmap is what fresh synthesis would have built; more often, fresh synthesis surfaces themes or items the iterative version has lost.

## Routing into systems of record

**Primary roadmap tool.** Strategic roadmap synthesis becomes source of truth. For tools with item-level structure (Productboard, Aha, [Linear](/integrations/overview#linear)), each Now and Next item becomes a feature/initiative in the tool. The roadmap is the document; the tool is the operational view.
...
**Earmark archive as source.** Source meetings stay in Earmark. The roadmap links back. When someone asks *"why is this on the roadmap?"*, the answer is traceable to the customer calls and strategy discussions that informed it. Anyone in the roadmap project can put that question to project chat and get the answer with sources.
...
- [Ad-hoc Q&A](/workflows/ad-hoc-qa-workflow) — for asking questions across meetings
- [Projects](/earmark-basics/projects) — for collecting and sharing the roadmap's source meetings
- [Chat](/earmark-basics/chat) — for running the synthesis across those meetings
- [Linear integration](/integrations/overview#linear) — for engineering tickets that flow from the roadmap
...
- [Local transcripts](/earmark-basics/local-transcripts) — for running your own analysis on very large corpora
