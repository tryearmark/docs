---
title: "Action items"
description: "Turn every meeting into a clean list of next steps with owners and due dates, and use Earmark Chat to catch commitments that slipped through."
...
Send the action items to the task system you actually open. The fewer steps between cleanup and "in my task system," the more reliable this workflow becomes. The [Linear integration](/integrations/overview#linear) generates one ticket per action item; for other task tools, the structured format is designed to paste cleanly.
</Step>
</Steps>
...
The big one for this workflow: the template doesn't roll up. Each meeting's action items live in that meeting's artifact, and nothing merges them into one running list for you.

For the cross-meeting view:
...
- **Ask [Chat](/earmark-basics/chat) what slipped.** "What did I commit to this week?" or the **Today's action items** suggestion on the Chat home page pulls commitments out of every meeting in range, with source chips back to the moment each was made. It's the fastest answer to "did I commit to something this week that I forgot?" On desktop recordings your mic is kept separate, so it can find your own words.
- **Find a specific meeting** with the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`) when you already know which call it was.
- **Export [local transcript files](/earmark-basics/local-transcripts)** if you want to run your own analysis across a week's worth of meetings.
...
To see how next steps have moved across the whole deal, keep the account's calls in a [project](/earmark-basics/projects) and ask from the project page: *"What next steps have we agreed with Acme, and which are still open?"*
