...
description: "Turn customer calls, partner meetings, and kickoffs into warm, specific follow-up emails that honor what was promised and go out the same day."
...
You don't have to hold that history in your head. Click **Chat** in the meeting's header and ask *"Did this come up in the previous meeting?"*, or ask [Chat](/earmark-basics/chat) *"What have I promised Acme across our last few calls, and what's still open?"* The answer cites each call, so you can check the exact wording before you reference it in an email. If you keep a [project](/earmark-basics/projects) per customer, ask from the project page and Chat stays on that customer's calls.
...
For implementation engagements with several customer touchpoints in a short window, a single consolidated update sometimes beats three separate recaps. Put the engagement's calls in one [project](/earmark-basics/projects) and ask from the project page: *"What did we cover and commit to across these calls, and what's still open on each side?"* Draft the email from that answer, or paste it into a **Customize context** document and run Template A against it. Either way, the personalization pass still applies: confirm each commitment against its source chip before it goes in an email.
...
**Reference prior emails in future emails.** Two emails into a relationship, refer to the first. Five emails in, reference the arc. This is how a sequence of follow-ups becomes a relationship rather than a series of transactions. *"When we first spoke in February, you mentioned X. Here's how that's evolving…"* — the customer feels the continuity. Use the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`) to jump to a prior conversation by title, or ask [Chat](/earmark-basics/chat) *"What did Acme say about X when we first spoke in February?"* to get the answer with a source. Name the month so Chat looks back far enough.
...
- [Projects](/earmark-basics/projects) — for keeping each customer's calls together so Chat can answer across them
