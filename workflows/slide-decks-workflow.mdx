---
title: "Slide decks"
description: "Turn strategy discussions, status reviews, and customer calls into slide-by-slide deck content with a narrative arc, speaker notes, and evidence."
...
Many decks draw from multiple source conversations. A board deck synthesizes across a quarter's worth of leadership meetings; a pitch deck pulls from customer interviews, strategy discussions, and traction reviews; an all-hands strategy deck combines exec discussions, customer signal, and team feedback.

**Put the source meetings in a project.** When you know multiple meetings will feed a deck, add them to a [project](/earmark-basics/projects) as they happen — a meeting can join before it starts, while it's live, or after. If teammates ran some of the source conversations, share the project so they can add theirs. Then ask from the project page: *"What did we decide this quarter, and what's still open?"* or *"Draft the narrative arc and slide headlines for a board update from these meetings."* [Chat](/earmark-basics/chat) answers with source chips, so every claim that makes it onto a slide can be traced to the meeting it came from. Treat the answer the way you treat any first draft: run the curation pass below before building slides.

If you'd rather run the saved template, ask Chat for the substance (decisions, customer language, numbers mentioned, open risks), paste the answer into a **Customize context** document, and run the template against it.

**Watch for contradictions.** Multi-meeting synthesis often reveals tensions — the strategy discussion said one thing; subsequent customer calls said another; the product roadmap implied one trajectory; the financial review surfaced a different reality. Ask the project directly: *"Where do these meetings disagree?"* The deck should resolve these tensions, not paper over them. Sometimes the right move is to acknowledge the tension in the deck itself — that's often the most honest and well-received content.
...
For an annual review or a pitch that draws on older conversations, name the period (*"since January"*) or say *"all my meetings."* Check the **Coverage** line at the end of each answer to confirm the date range and how many meetings it reviewed.

If you want to run your own analysis instead, export [local transcripts](/earmark-basics/local-transcripts) and run an external agent over them.
...
Source meetings stay in Earmark. Decks link back. When a future deck-builder asks *"where did this content come from?"*, the source is one click away. This is especially valuable for recurring decks — the Q4 board deck inherits content patterns from Q3, which inherits from Q2, all linked to source meetings. Keep one project per recurring deck (the board series, say), and next quarter's builder can start by asking it *"What's changed since the last board meeting?"* The team's presentation tradition becomes navigable.
...
- [Projects](/earmark-basics/projects) — for grouping a deck's source meetings and asking questions across them
- [Local transcripts](/earmark-basics/local-transcripts) — for running your own synthesis outside Earmark
