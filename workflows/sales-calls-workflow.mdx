...
Open the call in Earmark before it starts and add your saved Sales Discovery template as a task. Click the project chip next to the title and add the call to the account's [project](/earmark-basics/projects). See [Before a meeting](/using-earmark/before-a-meeting).
</Step>
...
To pull that together without digging, ask the account's project *"What's changed since our last call, and what risks are still open?"* With your calendar connected, you can also ask [Chat](/earmark-basics/chat) *"Help me prepare for tomorrow using my past meetings"*, and it pairs each call on your calendar with what was said in related past meetings.
</Step>
...
The deal runs across many calls, so the read on it has to as well. Two features take the workflow past the single call.

**A project per account.** Keep every call with one account in a [project](/earmark-basics/projects): discovery, demo, pricing, security review. If an SE or CSM runs calls on the same account, share the project with them as **Collaborators** so they add theirs. Everyone in the project can then ask questions across all of its calls, including ones they can't open.

**Chat for the deal-level rollup.** Before an executive sponsor meeting, a pricing conversation, or a forecast call, ask the account's project:

- *"What has Acme said about budget and timeline across our calls?"*
- *"How has the champion's engagement changed since discovery?"*
- *"Which objections are still unresolved?"*

[Chat](/earmark-basics/chat) answers with a source chip for each call, so you can check a claim before it goes in front of your manager. What used to be fifteen minutes of pasting CRM-update blocks into one document is now a question. Apply the same Stated / Implied / Unknown discipline to the answer that you apply to the artifact.

**The team view.** Add calls to a shared team project as well, like "Q3 new business"; a meeting can be in several projects. A manager in that project can ask *"What are the most common objections across these deals?"* and get an answer drawn from every call the reps have added.

The rest still applies:

- **Find a specific call** by title with the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`).
- **The CRM is the pipeline of record.** If every call's CRM-update block lands in the opportunity activity log, the CRM holds the deal's history in structured form. The rep's job is to feed the CRM faithfully; the manager's filters in the CRM are what produce pipeline visibility.
- **Run your own analysis** on [local transcript files](/earmark-basics/local-transcripts) if you want an external agent to process an entire deal's worth of calls in a pipeline you control.
...
- **Treating discovery as a one-call event.** MEDDPICC dimensions get filled in over multiple calls. "Unknown" on call one and still Unknown on call four is a different problem than "Unknown" → "Strong read" by call four. Track the trajectory: carry prior reads into the next call with Customize context, or ask the account's project how the read has changed.
...
- **Letting deals age in stage without a fresh rollup.** A deal in "Demo" for five weeks needs a fresh deal-level synthesis (ask the account's project for one). Otherwise it's running on stale signal.
...
- [Projects](/earmark-basics/projects) — one project per account, shared with the account team
- [Chat](/earmark-basics/chat) — deal-level questions across an account's calls, and prep for upcoming ones
