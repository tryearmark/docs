...
Leadership readouts are by definition rollups across many meetings. The slow part used to be the gathering: finding the week's meetings, copying out what mattered, pasting it together. [Chat](/earmark-basics/chat) does that part now. Ask *"Give me a recap of my meetings this week"* for the weekly digest, or *"What did we decide this month, and what risks came up?"* for a business review. Chat cites each meeting it drew from, and every answer about your meetings ends with a **Coverage** line — the date range it searched and how many meetings it reviewed — so you can see whether the rollup is complete before you build on it.

Two things haven't changed. Chat only sees meetings: the metrics, written function-lead updates, and Slack signal still come from you, and the four templates below still run on a **Customize context** document where you bring those together. And the model still produces the draft, not the readout. The synthesis — what to elevate, what to cut, how to calibrate — is the writer's curation work. Most of this guide is about doing that part well.
...
A scope rule written down once protects every future artifact from drift. Without it, every readout tries to include everything, and signal density collapses. If you keep the series' meetings in a [project](/earmark-basics/projects), put the scope rule in the project's **Instructions** too; Chat follows them in every answer in that project.
...
For the weekly digest, gather the inputs as described below, paste them into a new task, and iterate the prompt in the [Composer](/using-earmark/composer) until the draft lands.
</Step>
...
For the weekly digest, gather:

- The meeting side (leadership meetings, all-hands, key project meetings, customer escalations) from [Chat](/earmark-basics/chat). Ask *"Give me a recap of my meetings this week"* or *"What decisions did we make this week, and what's at risk?"* Check the **Coverage** line against your calendar, and hover the source chips on anything that will become a headline. Use the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`) when you want to open a specific meeting's artifact.
...
A shared leadership [project](/earmark-basics/projects) makes the meeting side far more complete. Add function leads as **Collaborators** and they add their own review meetings. You see each meeting's title, who recorded it, and when, but not the meeting itself; Chat can still use those transcripts and notes to answer your questions in the project.
</Step>
...
<Tip>
Triangulate signal in the synthesis. The highest-value content in a leadership artifact often lives in tensions: a function lead's update says "on track" while customer call artifacts say customers are frustrated; the sales pipeline view says strong while recent deal artifacts say champions are weakening. Look for these tensions deliberately when assembling inputs. Ask Chat for them directly, too: *"Where did this week's meetings disagree with each other?"* They're invisible to single-source summaries; surfacing them is what makes the readout earn the read.
</Tip>

## What this workflow doesn't do

Chat handles the cross-meeting gathering. The [ad-hoc Q&A workflow](/workflows/ad-hoc-qa-workflow) covers how to ask it good questions. A few limits still shape this workflow:

- **Chat only sees meetings it has access to.** It uses meetings you recorded and meetings shared with you through projects. A function review nobody added to the leadership project won't be in the rollup, and the **Coverage** line tells you how many meetings were reviewed, not whether they're the ones you expected.
- **Name the period.** Say which week, month, or quarter you mean. For anything older, name the period or ask about "all my meetings."
- **It doesn't do the curation.** Chat can make mistakes, and it has no view on what leadership needs to hear this week. The headline, the calibration, the cuts, and the leak test are the writer's.

The gathering step shrinks; the 20-minute curation pass doesn't. That's the right trade: the curation is where the value is.

If you want to run your own analysis on larger sets (a year of leadership meetings, say), [export local transcripts](/earmark-basics/local-transcripts) and run an external agent over them.
...
Pull the relevant past readouts from the archive, then ask [Chat](/earmark-basics/chat) what actually happened on each flagged item: *"What happened with the enterprise SSO risk since April?"* Name the month when you're reaching back further. Pair each prediction with the answer and work through them by hand. Cheap, high-leverage, and rarely done.
...
- **Skipping the Coverage line.** A weekly recap that reviewed six meetings when the week had eleven is a digest with holes. Check it before you draft. If meetings are missing from the project, add yours and ask whoever recorded the others to add theirs.
...
- [Chat](/earmark-basics/chat) — weekly and monthly recaps across your meetings, with sources and a Coverage line
- [Projects](/earmark-basics/projects) — for keeping a readout series' meetings together and sharing them with function leads
- [Ad-hoc Q&A workflow](/workflows/ad-hoc-qa-workflow) — for asking good questions across meetings
...
- [Command menu](/earmark-basics/meeting-history#search-with-the-command-menu) — for finding the right meetings to assemble as input
- [Local transcripts](/earmark-basics/local-transcripts) — for export when you want to run your own synthesis
