...
description: "Turn a great product demo, training session, workshop, or research interview into a repeatable script that anyone on your team can run just as well."
...
The workflow is unusually tractable today: each script comes from a single source session captured in a normal Earmark recording. No cross-meeting synthesis required for the core flow. The "cross-team synthesis" enhancement (combining moves from multiple sources) is covered as an optional step toward the end.
...
- **Multiple sessions across the team** — synthesize the best moves from each (see the iteration section)
...
Long workshops hit the [recording limit](/earmark-basics/auto-end#recording-limits): capture stops at 3 hours, with a notice at 2.5. Click **Start capture** in the same meeting to keep the whole session in one transcript. Otherwise the script loses its close.
</Step>
...
This cross-team synthesis is the part of the workflow that benefits from cross-meeting context. Give each script a [project](/earmark-basics/projects) and share it with everyone who runs the activity. Each person adds their own recordings of it (you can only add meetings you recorded). Before the quarterly session, the script owner asks the project chat: *"Across these demos, which beats and phrasings drew the most engagement, where did presenters depart from the script, and what questions came up that the script doesn't cover?"* People in the project can't open each other's recordings, but [Chat](/earmark-basics/chat) answers from all of them and shows short excerpts as sources.
...
- [Projects](/earmark-basics/projects) — keeping a script's source session and later runs together for cross-team synthesis
