...
Then live by it. Honor "let's go off-record" immediately. For sessions you want captured without keeping long-term, use [temporary meetings](/earmark-basics/temporary-meetings) — the meeting isn't kept, so Chat can't draw on it later. For some conversations, don't record at all: compensation delivery, terminations, mental health discussions. Presence matters more than artifact for these.
</Warning>
...
- **Workspace isolation.** Meeting content is private per user — your 1:1 artifacts are not visible to anyone else in your workspace unless you share them. That includes Chat: a teammate's Chat never uses your meetings unless you add them to a shared [project](/earmark-basics/projects). Keep 1:1s out of shared projects. See [Security and privacy](/security-and-privacy).
...
<Step title="Prep from past 1:1s">
The day before, open [Chat](/earmark-basics/chat) and ask *"Help me prepare for my 1:1 with Priya tomorrow using our past 1:1s."* With your [calendar connected](/getting-started/calendar), Chat pairs the upcoming 1:1 with your earlier ones. Ask it for the open threads and both sides' commitments, and each comes back with a source. It's the fastest way into the continuity loop below.
</Step>
...
- **Confirm the Open threads section.** This is what carries to next week. Add anything the model missed. If you're not sure whether a thread is new, click **Chat** in the meeting header and ask *"Did this come up in the previous meeting?"*
...
## Working across many 1:1s

The biggest moves managers want — **performance review prep**, **promotion case building**, **interview calibration across a hiring loop**, **disengagement detection across a quarter of 1:1s** — all draw on many meetings. Ask [Chat](/earmark-basics/chat) from the sidebar. It searches the meetings you recorded, cites each one, and ends with a **Coverage** line showing the date range and how many meetings it reviewed. Check that count against the 1:1s you actually had. Name the review period when you ask. Your chats are private to you.

- **Performance review prep.** *"Across my 1:1s with Priya since January, list her open threads, the commitments each of us made and whether they closed, and anything that touched on growth or feedback. Cite each meeting."* Then read the sources before anything goes into the review. The synthesis gives you the material; the assessment is yours.
- **Promotion case building.** Same pattern, oriented to the competencies at the target level. Ask for specific moments and her own words. The structure of the 1:1 template (specific moments, verbatim quotes, growth notes) is what makes those answers precise.
- **Interview calibration across a loop.** Each interviewer adds their interview to a shared project for the loop, and project chat synthesizes across them. See the [hiring workflow](/workflows/hiring-workflow).
- **Disengagement detection.** Ask about what was said, not how anyone felt: *"Over my last quarter of 1:1s with Priya, which topics did she raise each time, and which threads have stayed open for more than three meetings?"* Then read the artifacts it cites. A pattern is a reason for a conversation, not a conclusion. If you'd rather run your own analysis, export [local transcript files](/earmark-basics/local-transcripts) for the period.
- **Find a specific past 1:1** with the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`). To find when a topic first came up, ask Chat, and say "ever" so it searches your full history: *"When did Priya first raise the principal track, ever?"*
...
- **Performance reviews written from recent memory anyway.** If you have six months of artifacts and still write the review from the past three weeks, you have defeated the purpose. Ask Chat across the whole review period first.
- **Adding 1:1s to a shared project.** Everyone in a project can ask Chat about its meetings and see short excerpts. Keep 1:1s and skip-levels out of shared projects entirely; interviews go only in their own loop's project.
...
- [Chat](/earmark-basics/chat) — prep from past 1:1s and synthesis across a review period
