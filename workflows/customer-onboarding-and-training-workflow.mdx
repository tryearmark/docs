...
description: "Turn onboarding calls and training sessions into follow-ups that track every commitment, configuration need, feature request, and documentation gap."
...
| Engineering team | Tickets in [Linear](/integrations/overview#linear), Jira, or GitHub via the [meeting-to-tickets workflow](/workflows/meeting-to-tickets-workflow) |
...
Give every customer implementation its own [project](/earmark-basics/projects) and put every session in it, kickoff through hypercare. The project is what makes the cross-session views later in this guide one question instead of an afternoon of copy-paste.

- **Share it with everyone on your side** who runs sessions with that customer (CSM, EIC, solutions engineer) as **Collaborators**, plus the implementation lead. Collaborators can use the project in Chat and add their own meetings.
- **Each person adds the sessions they recorded.** You can only add meetings you created. Add a session before it starts, from the project chip next to the meeting title, so it never gets forgotten afterward.
- **Backfill an implementation already under way** from the **Meetings** page: tick the past sessions (Shift-click for a range, up to 50), then click **Add to project** in the toolbar.
- **Put your categorization model and the "only unresolved" rule in the project's Instructions** (under **Project settings**). Chat follows them for every question asked in the project.

People in the project see each session's title, who recorded it, and when. They can't open each other's sessions, but [Chat](/earmark-basics/chat) can use them to answer questions in the project and shows short excerpts as sources.
...
For looking at one customer across all their training sessions. Run before milestones (pre-go-live, end-of-onboarding, mid-implementation review) or whenever the team needs a snapshot of where this customer stands. Paste it as your question in the customer's project chat; it answers from every session in the project, including the ones teammates ran.

```md
Synthesize across this customer's training sessions. Produce an
implementation snapshot showing where the customer stands now: what's
been resolved, what's still open, what's at risk, what's coming next.
...
For the implementation team lead. Run quarterly (or monthly for higher-volume teams) across all customer implementations to surface patterns. Paste it as your question in [Chat](/earmark-basics/chat) from the sidebar, which searches the sessions you recorded plus every session shared with you through a project. That's why the lead belongs in every customer project: sessions in projects you're not in are invisible to your Chat. Name the period you mean, and check the **Coverage** line for how many sessions it reviewed.

```md
Synthesize across the training sessions from {date range}, across all
customers. Identify patterns worth acting on at
...
For trainers running multiple sessions per day, the template attachment becomes a recurring habit. Some teams set up the template as the default for the entire training calendar series; others attach per-session. Either way, set it before the session starts so the artifact is ready when the session ends. See [Before a meeting](/using-earmark/before-a-meeting). While you're there, add the session to the customer's project from the project chip next to the meeting title.

On the desktop app, [Auto-end](/earmark-basics/auto-end) stops each recording once the session is over, so back-to-back sessions don't bleed into each other's queue. Sessions that run long keep recording.
</Step>
...
If another team member runs the next session with this customer, share the artifact with them ahead of their session. They start with the customer's open items already in their head; the customer experiences continuity. They can also ask [Chat](/earmark-basics/chat) *"Help me prepare for tomorrow's session with Acme using past sessions"*: with their calendar connected, it pairs the upcoming session with past ones, including sessions teammates added to the customer's project.
</Step>
</Steps>
...
Single sessions are tactical. The cross-session view per customer is what the implementation team needs to manage the customer's overall trajectory. Run Template C in the customer's project chat. The answer cites sessions with source chips; hover one to see the excerpt it came from. Check the **Coverage** line: the meeting count should match the sessions you've run. If it's short, someone's sessions aren't in the project yet.
...
**Renewal and expansion preparation.** Before a renewal conversation, run Template C across the customer's full implementation history. Implementations often run for months, so ask for the full history explicitly. The artifact tells the renewal lead what was promised vs. delivered, what's still pending, where enthusiasm was strongest, where friction was raised, what stakeholders are most engaged. A renewal conversation grounded in this artifact is materially different from one grounded in memory.
...
**The quarterly pattern report.** Once a quarter, the implementation lead runs Template D in Chat across every customer project they're in. The output:
...
If you want to run your own analysis, [export local transcripts](/earmark-basics/local-transcripts) and run an external agent on them.
...
**Engineering intake.** Bug reports and engineering-blocking items routed via the [meeting-to-tickets workflow](/workflows/meeting-to-tickets-workflow) and the [Linear integration](/integrations/overview#linear).

**CSM / Account record.** The CRM or CS platform's account record tracks the customer's overall trajectory.

**Earmark archive as source.** Source meetings stay in Earmark. Every artifact links back. When a question arises six months later — *"what did we agree on about {topic} during the configuration phase?"* — ask it in the customer's project chat and you get the answer with the excerpt it came from, even if a colleague ran that session. Especially valuable during renewal conversations and during escalations.
...
**Onboarding retrospective.** Within 30 days of an implementation completing, run a brief retrospective in the customer's project chat, across the full implementation. What did we promise? Did we deliver? What items took longest to resolve? Why? What patterns showed up that should inform the next implementation? What did we learn about the customer that should inform their post-launch engagement? What feedback should go to product, docs, engineering? The retrospective is what feeds learnings forward.
...
- **Sessions missing from the customer's project.** Project chat focuses on the sessions in the project. A trainer who skips adding theirs leaves a hole in every snapshot. Add sessions before they start.
...
- [Projects](/earmark-basics/projects) — one per customer, shared with everyone who runs their sessions
- [Chat](/earmark-basics/chat) — per-customer snapshots and cross-customer patterns, with sources
...
- [Linear integration](/integrations/overview#linear) — for engineering tickets
...
- [Local transcripts](/earmark-basics/local-transcripts) — if you want to run your own analysis outside Earmark
