...
description: "Turn recurring working meetings into a durable record of what was discussed, decided, committed to, and still open, meeting after meeting."
...
- **Todos** → your task system. The [Linear integration](/integrations/overview#linear) generates one ticket per todo with a link back to the artifact; for other task tools, paste.
...
<Note>
Templates work within a single meeting — the template doesn't pull last meeting's todos into this one on its own. To make the continuity loop work, paste last meeting's todos and open questions into the **Customize context** dialog before this meeting starts. The template instructs the model to use that context as the status section. To gather them without copying by hand, ask [Chat](/earmark-basics/chat): *"List the open todos and open questions from last week's leadership sync."*
</Note>
...
Earmark generates and refines artifacts inside a single meeting. The template won't pull last week's todos into this week's artifact automatically, and it doesn't keep a running decision log for you.
...
- **"Did we ever decide X?":** ask [Chat](/earmark-basics/chat). It answers across meetings with source chips back to the moment the decision was made, and a **Coverage** line showing how far back it looked. Use the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`) when you only need to find and open a specific meeting.
- **A series-level view:** add every session of a recurring meeting to a [project](/earmark-basics/projects) and ask from the project page — *"What decisions have we made?"* or *"What's the latest status?"* Share the project with the team, and everyone can ask the same questions of the same set of meetings.
- **If you want to run your own analysis:** export [local transcript files](/earmark-basics/local-transcripts) and run external synthesis, for example a quarterly review in your own format.
...
- [Projects](/earmark-basics/projects) — keeping a recurring series together so Chat can answer across its sessions
