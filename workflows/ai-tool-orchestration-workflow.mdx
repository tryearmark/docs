...
description: "Use Earmark as the context layer for your AI tools, turning meetings into well-shaped prompts for Claude Code, Cursor, v0, Perplexity, and more."
keywords: ["AI prompts", "Claude Code", "Cursor", "v0", "Perplexity", "Obsidian", "system prompt", "AI orchestration", "workflow"]
---

The AI tool landscape has fragmented. Where a year ago you had ChatGPT and Claude as general assistants, today you have a dozen specialized tools: Claude Code and Cursor for engineering, v0 for React components, Lovable and Bolt for full-stack apps, Figma Make for UI prototypes, Perplexity for research, Notebook LM for source-grounded analysis, and Obsidian for durable markdown notes and linked knowledge bases. Each tool is dramatically better at its specialty than a general assistant. Each, used well, can produce real artifacts that previously required a specialist.
...
| --- | --- | --- |
| Code generation | [Claude Code / Cursor / Aider](/integrations/overview#cursor) | Code generation, refactoring, multi-file changes |
| UI components | [v0 by Vercel](/integrations/overview#v0) | React components, Tailwind UI |
...
| Coded engineering tasks | [Codex](/integrations/overview#codex) | Engineering tasks shaped by architecture docs |
| Engineering tickets | [Linear](/integrations/overview#linear) | Tickets that flow from architectural decisions |
...
Earmark has direct integrations with Cursor, v0, Codex, and Linear (linked above). The rest are paste targets — Earmark generates the prompt; you paste into the tool. When the prompt comes from a [Chat](/earmark-basics/chat) answer, **Handoff to Claude Cowork** and **Handoff to Codex** open your question and the answer in that app directly, with no copy and paste.
...
**Obsidian** — markdown-native output; `[[wikilinks]]` to related notes where cross-referencing helps; YAML frontmatter when metadata matters (meeting date, project, tags); folder or MOC structure the vault already uses. For AI plugins in the vault (Copilot, Smart Connections, and similar), paste shaped prompts the same way you would into Claude or ChatGPT — the vault is the context layer; Earmark supplies the meeting-grounded prompt.
...
The context a system prompt needs (what you're focused on, who you work with, the terms your team uses) rarely lives in one meeting. Rather than assembling it by hand, paste Template D into [Chat](/earmark-basics/chat) and ask it to draw on your recent meetings, or run it from a [project](/earmark-basics/projects) page if the assistant is for one body of work. Rerun the same question at each quarterly revisit to see what's changed.
...
## When the context spans several meetings

The templates above run on one meeting. Often the context a good prompt needs is spread across several: a feature discussed over three design reviews, a research question that came up in five customer calls. Use [Chat](/earmark-basics/chat) for these.

<Steps>
<Step title="Ask Chat with the template">
Open **Chat** from the sidebar, paste the template that matches the downstream tool, and say which meetings to draw on: *"Using our design reviews on the import flow from the last month, write a Claude Code prompt in this format."* If the work already has a [project](/earmark-basics/projects), ask from the project page so Chat focuses on that project's meetings, including ones teammates shared.
</Step>
<Step title="Check the sources">
Hover the source chips to confirm each requirement came from a real conversation, and read the **Coverage** line to see how many meetings Chat reviewed. Name an older period if the context goes back further.
</Step>
<Step title="Hand off or copy">
For Claude Cowork or Codex, **Handoff to Claude Cowork** or **Handoff to Codex** opens your question and the answer in that app, ready to keep working. For every other tool, **Copy** keeps the formatting and drops the source chips. Handoff sends the answer as it is, so if it needs scrubbing for sensitive content, copy and edit it instead.
</Step>
</Steps>

If you want to run your own pipeline instead, export [local transcript files](/earmark-basics/local-transcripts) and point an external agent at them.
...
- [Cursor integration](/integrations/overview#cursor) — for coded implementation
- [v0 integration](/integrations/overview#v0) — for UI components
- [Codex integration](/integrations/overview#codex) — for engineering tasks
- [Linear integration](/integrations/overview#linear) — for tickets that flow from AI-tool work
...
- [Chat](/earmark-basics/chat) — for prompts that draw on several meetings, and for handing answers off to Claude Cowork or Codex
- [Projects](/earmark-basics/projects) — for keeping one body of work's meetings together so Chat can draw on them
- [Local transcripts](/earmark-basics/local-transcripts) — for export when you want to run your own pipeline over the transcripts
