...
- **Access is narrow.** Artifacts are for the hiring loop, the recruiter, and the hiring manager. Not part of company-wide ad-hoc Q&A. See the [ad-hoc Q&A workflow](/workflows/ad-hoc-qa-workflow) on the privacy discipline. If you run the loop in a shared [project](/earmark-basics/projects), everyone in it can ask Chat about every interview in it, so keep the project to the loop.
...
Use [temporary meetings](/earmark-basics/temporary-meetings) for sessions that shouldn't be kept at all. They're never stored on Earmark's servers and can't be added to a project, so they won't be part of loop-level synthesis. Earmark's [workspace isolation](/security-and-privacy) keeps interview artifacts private to the interviewer until they're shared.
</Warning>
...
For the interview type and the specific competencies being evaluated. See [Before a meeting](/using-earmark/before-a-meeting). If the loop has a project, add the interview to it now from the project chip next to the meeting title. See [Loop-level synthesis with a project](#loop-level-synthesis-with-a-project).
</Step>
...
## Loop-level synthesis with a project

Each interview artifact stands on its own. The debrief needs them side by side. Give each loop a [project](/earmark-basics/projects), and [Chat](/earmark-basics/chat) can synthesize across every interviewer's meeting without anyone forwarding notes.

<Steps>
<Step title="Create one project per loop">
The hiring manager or recruiter creates it and stays the owner. Name it for the candidate and role. In **Project settings**, paste a short version of the rubric into **Instructions**: the competencies, the target level, and the rule to cite evidence and write "Unknown" rather than infer. Chat follows those instructions for every question asked in the project.
</Step>
<Step title="Share it with the loop, and only the loop">
Click **Share** and add each interviewer as a **Collaborator**. Collaborators can use the project in Chat and add their own meetings; only owners manage people and settings. Anyone in the project can ask Chat about every interview in it, so add only the people who'll be in the debrief. Hiring conversations are sensitive: don't reuse a broad team project for them.
</Step>
<Step title="Each interviewer adds their own interview">
You can only add meetings you recorded, so every interviewer adds their own, from the project chip next to the meeting title. It works before, during, or after the interview. Screens and reference checks go in the same project.
</Step>
</Steps>

What people in the project can see is narrower than it sounds. They see each interview's title, who recorded it, and when. They **can't open** each other's meetings. Chat can use the transcripts and notes to answer questions in the project, and shows short excerpts as sources. Only the interviewer who recorded a meeting can jump into it from a source chip.

**Write your own scorecard before you ask the project anything.** Project chat can tell you what the other interviewers saw. Reading that before you've committed to your own read is the same anchoring problem as speaking second in the debrief. Push to the ATS first, then use the project.

Before the debrief, the hiring manager asks the project:

```md
Across the interviews in this project, for each competency in the rubric:
- Each interviewer's read, with the strongest piece of evidence
- Where interviewers diverged, and what each one saw
- Every gap flagged Coachable or Barrier, and whether the loop agrees
- Competencies with insufficient signal across the whole loop
Cite the interview for every point. Do not make a hire recommendation.
```

Check the **Coverage** line at the end of the answer. It should count every interview in the loop; if it's short, someone hasn't added theirs yet. Use **Copy** to move the answer into the debrief doc. This is the substrate the debrief actually needs.
...
A debrief run this way converges in 25–30 minutes instead of the usual 60. Most of the work has already been done by the structured artifacts and the loop synthesis; the debrief converges on the divergences and the decision.

## What this workflow doesn't do

Loop-level synthesis is covered by the loop's project. Two longer-horizon moves still take a deliberate habit, and one handoff stays manual:

- **Post-hire retrospectives at 6 and 12 months.** If the loop's interviews are still within your retention policy, ask [Chat](/earmark-basics/chat) from the sidebar, not from the loop's project. It can see the loop's interviews (shared with you through the project) and your own 1:1s with the new hire, so you never need to add 1:1s to the hiring project. Name the period: *"Compare what the loop said about Priya's system design in her March interviews with what's come up in my 1:1s with her since she started."* What did the loop predict? What's actually happening? Which coachable gaps closed? If you'd rather run your own analysis, pair the original artifacts with recent 1:1 artifacts from the [people and team meetings workflow](/workflows/people-and-team-meetings-workflow) and run an external synthesis on the pair.
- **Rubric refinement.** Quarterly, the talent lead or hiring manager reviews the post-hire retrospectives and updates the rubric, the question bank, and the coachable-vs-barrier list. Chat can gather the material; the judgment about what to change stays with you. The rubric becomes a living document over years.
...
The compounding payoff — measurably more accurate rubrics, calibrated interviewers, question banks that correlate with performance — requires this loop to be run. Most companies never realize it because the artifacts to support it never exist. With this workflow, they do.
...
- **Anchoring in the debrief.** The first speaker tends to anchor the room. Mitigate by collecting individual recommendations before discussion — in writing if possible. The same applies to the loop's project: write your scorecard before you ask Chat what anyone else saw.
...
- **Querying interview artifacts for ad-hoc Q&A.** Interview artifacts are restricted access. Not part of company-wide [ad-hoc Q&A](/workflows/ad-hoc-qa-workflow) corpora. Keep them inside the hiring loop: add interviews only to that loop's project, never to a broad team project, and share the project only with people in the loop.
...
- [Projects](/earmark-basics/projects) — sharing a loop's interviews, roles, and what people in a project can see
- [Chat](/earmark-basics/chat) — asking questions across the loop, with sources
- [Temporary meetings](/earmark-basics/temporary-meetings) — for sessions that shouldn't be kept at all
