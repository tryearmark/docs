...
description: "Turn discovery calls, kickoffs, design reviews, and technical discussions into a one-pager, full PRD, or technical spec engineers can build from."
...
A full PRD frequently draws from multiple meetings. The synthesis section below covers how to pull them together.
...
The PRD usually draws from multiple meetings. See the multi-meeting synthesis section below for how to run it across all of them.
...
The challenge: producing a coherent PRD from a corpus of conversations spread across weeks. The way to do it is a [project](/earmark-basics/projects) for the feature, with [Chat](/earmark-basics/chat) doing the synthesis:

- **Give the feature a project.** Add each source meeting as it happens; meetings can join before they start, while live, or after. Use the [command menu](/earmark-basics/meeting-history#search-with-the-command-menu) (`Cmd+K` / `Ctrl+K`) or **Search meetings** on the **Meetings** page to find older ones. You can only add meetings you recorded, so **Share** the project with the tech lead and designer and ask them to add the reviews and scoping sessions they ran.
- **Put your PRD format in the project's instructions.** In **Project settings**, add your team's required sections to **Instructions**, so drafts in the project follow them.
- **Run Template B in project chat.** From the project page, paste Template B into the chat box. The answer draws across the project's meetings, cites them with source chips, and ends with a **Coverage** line showing how many meetings were reviewed.
- **Ask for decisions directly.** *"What decisions have we made on the import flow, and in which meeting?"* is a faster check on the draft's decided / proposed / open markings than rereading every artifact.

If you'd rather run your own synthesis, the older paths still work: paste the relevant artifacts from each source meeting into a single **Customize context** document and run Template B as a task, or export [local transcript files](/earmark-basics/local-transcripts) and run an external agent across them.

The most useful signal from multi-meeting synthesis is **contradiction**. The customer described the problem one way; the internal kickoff framed it differently. The design review proposed a solution shape the technical scoping says is infeasible. The stakeholder review approved a direction that customer follow-ups have since complicated. These contradictions are the most important content the PRD can surface. Either they get resolved (and the resolution is captured) or they remain open (and the PRD makes the openness explicit). A PRD that papers over real contradictions ships a project that runs into them later, more expensively. Ask project chat for them directly: *"Where do the customer calls and the kickoff describe the problem differently?"*

Use Template D after every meaningful project-related meeting to keep the PRD current, and add that meeting to the project so later questions include it. The PRD becomes a living artifact — v1 from the one-pager, v2 incorporating design review feedback, v3 incorporating technical scoping, and so on, each with a change log.
...
Within a month of shipping, run a brief retrospective using the PRD as the substrate: Did we ship what the PRD specified? Where did we drift, and why? Did the success metrics move? Were the risks accurate? What did we learn that should update our future PRDs? If the feature's meetings are in a project, project chat can list the decisions made after the PRD was approved, which is usually where the drift started.
...
- [Projects](/earmark-basics/projects) — for keeping a feature's meetings together and sharing them with the team
- [Chat](/earmark-basics/chat) — for running the PRD synthesis across those meetings
