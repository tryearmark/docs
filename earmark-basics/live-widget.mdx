...
| **Pin** | Drops a [pin](/earmark-basics/pins) on the current moment, same as the meeting command bar or `Alt + P` |
...
<Step title="Start a recording from the main window">
The widget appears when a recording starts from the main Earmark window.
</Step>
</Steps>
...
<Note>
The Live Widget setting is **per machine**. Turning it on one computer doesn't turn it on elsewhere.
</Note>

<Warning>
The widget is tied to recordings started in the **main window**. If you start a recording from a secondary window, you'll see a toast explaining that the floating control appears for main-window recordings instead.
</Warning>

For global pin shortcuts that work even when the widget isn't visible, use **Settings > Keyboard shortcuts** (default `Alt + P` on desktop). See [Pins](/earmark-basics/pins) for when pinning is available and what happens after you drop one.
