...
<Frame caption="Type / in the prompt bar to filter templates">
  ...
  >
    ...
    >
      <img
        src="/images/template-shortcut.png"
        alt="Meeting prompt bar with slash command active and filtered template suggestions in the dropdown."
        style={{ width: '100%', display: 'block' }}
      />
    </div>
  </div>
...
When something lands in the conversation — a decision, a risk, a quote you need later — drop a [pin](/earmark-basics/pins) instead of breaking focus to take notes. Earmark captures the last ~60 words around that moment so you can turn it into a focused artifact after the call.

| Method | Best for |
| --- | --- |
| `Alt + P` (desktop, customizable) | Fast capture without switching apps — works even when Earmark isn't focused |
...
<Frame caption="Drop a pin during a live meeting">
  ...
  >
    <video
      ...
      title="Pinning an important moment during a live meeting."
      style={{ width: '100%', display: 'block' }}
    />
  </div>
</Frame>
...
<Tip>
Use pins liberally during the call — you can delete or edit titles later. Turn on **Include pins in artifacts** from the **⋯** menu in the Pins panel when you want new tasks to prioritize pinned context automatically.
</Tip>

Pinning requires an active recording, a signed-in account, and transcript text. [Temporary meetings](/earmark-basics/temporary-meetings) don't support pins. For limits, privacy, and the full capture flow, see [Pins](/earmark-basics/pins).
