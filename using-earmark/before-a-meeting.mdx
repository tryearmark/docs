...
description: "Prepare for meetings in Earmark: catch up with Chat, add the meeting to a project, seed tasks from templates, and add context before the call."
...
Before a recurring sync or a customer call, ask [Chat](/earmark-basics/chat) what you need to remember:

- **"Help me prepare for tomorrow using my past meetings."** With your [calendar connected](/getting-started/calendar), Chat looks ahead up to two weeks and pairs each upcoming event with what was said in related meetings.
- **"What did we decide last time with Acme, and what's still open?"** Chat answers from past meetings and cites the ones it used.
- **Open the meeting itself and click Chat** in the header, then choose **Help me prepare for this meeting**.

If the meeting belongs to a [project](/earmark-basics/projects), add it before the call starts. That keeps it with the rest of the project's context from the first minute.
...
<Frame caption="Pre-seeded tasks before recording starts">
  ...
  >
    ...
    >
      <img
        src="/images/preseed-task-meeting-minutes.png"
        alt="Meeting view with pre-added tasks such as meeting minutes before capture has started."
...
<Frame caption="Customize context for meeting background and goals">
  ...
  >
    ...
    >
      <img
        src="/images/meeting-customize-context.png"
        alt="Customize context dialog with fields for meeting background, goals, and key questions."
        style={{ width: '100%', display: 'block' }}
      />
    </div>
  </div>
...
- [ ] Ask Chat to catch you up on related past meetings
- [ ] Add the meeting to its project, if it has one
